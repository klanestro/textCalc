...
BOOL CEditReplaceDlg::DoHighlightText(BOOL bMsg)
{
	...
	if (! bFound)
	{
		if (bMsg) {
			CString prompt;
			prompt.Format(IDS_EDIT_TEXT_NOT_FOUND2, m_sText);
...
void CEditReplaceDlg::OnEditReplace() 
{
	...
	m_pBuddy->PasteSelection(m_sNewText,1);
...
void CEditReplaceDlg::OnEditReplaceAll() 
{
	...
	while (m_bFound)
	{
		...
		//VERIFY(m_pBuddy->ReplaceSelection(m_sNewText));
		//m_pBuddy->PasteSelection(m_sNewText,1);
		m_pBuddy->UnitPasteSelection(m_sNewText);
	...
	if (m_nScope == 0)
	{
		//TRACE("\nCheckState3()"); This state is called
		//also if, commment out this line...does not remove the undo can't find crash!
		m_pBuddy->SetSelection(m_ptBlockBegin, m_ptBlockEnd);
	}

	m_pBuddy->EndUnitPaste();
	TRACE("\nEndUnitPaste()");
