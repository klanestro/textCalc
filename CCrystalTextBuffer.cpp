...
BOOL CCrystalTextBuffer::InitNew(int nCrlfStyle /*= CRLF_STYLE_DOS*/)
{
	...
	m_savedUndoGroup = m_savedUndoBeginGroup = FALSE; //used exclusively only in beginundogroup
...
BOOL CCrystalTextBuffer::Undo(CPoint &ptCursorPos)
{
	...
	for (;;)
	{
		...
		{
			//TRACE("\nINSERTION UNDO RECORD! m_nUndoPosition=%d", m_nUndoPosition);
		...
		{
			TRACE("\nDELETION UNDO RECORD! m_nUndoPosition=%d", m_nUndoPosition);
		...
		//used to signify the end of a undo group...so break!
		if (ur.m_dwFlags & UNDO_BEGINGROUP) {
			TRACE("\nBEGINGROUP UNDO RECORD m_nUndoPosition=%d", m_nUndoPosition);
			break;
		}
	}
	...
	//Wrong! here! the bufsize and position should be the same should be the same
	//actually it is ok....the size is not zero because we still neeed redo...
	TRACE("\nAfter Undo : m_aUndoBuf.GetSize()=%d, m_nUndoPosition =%d", m_nUndoPosition , m_aUndoBuf.GetSize());
...
void CCrystalTextBuffer::AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText, int nActionType)
{
	...
	if (nBufSize >= m_nUndoBufSize)
	{
		...
		if (nIndex == nBufSize) 
		{ 
			//we assume this part/case never gets called in real work!
			//if however, it is reached, the program is able to cater for this case!

			//if the group of records to be removed is that large...we will be careful  not delete this record!
			//note that this case is reached is extremely rare is the undobufsize is set very big...
			//because previous blocks will usually be cleared to make way for this
			
			//this the same group of undo...so cannot remove records in the same group
			//do nothing...don't remove any records

		}
		else {
			
			//note the :  ASSERT(m_aUndoBuf.GetSize() < m_nUndoBufSize);
			//is no longer valid
			nIndex = 0;
			for (;;)
			{
	...
	if (ptStartPos==ptEndPos) {
		...
		//TRACE("\nAdding record with zero length!");
		...
		//TRACE("\nAdding Record: m_aUndoBuf.GetSize() %d , m_nUndoBufSize %d, m_nUndoPosition %d, m_bUndoBeginGroup %d" ,m_aUndoBuf.GetSize() , m_nUndoBufSize, m_nUndoPosition, m_bUndoBeginGroup);
	}
...
BOOL CCrystalTextBuffer::GetActionDescription(int nAction, CString &desc)
{
	...
	*/
	bSuccess = desc.LoadString(IDS_EDITOP_NOTSHOWN);
...
void CCrystalTextBuffer::BeginUndoGroup(BOOL bMergeWithPrevious /*= FALSE*/)
{
	...
	ASSERT(! m_bUndoGroup);
	m_savedUndoGroup = m_bUndoGroup;
	m_savedUndoBeginGroup = m_bUndoBeginGroup;
	
	m_bUndoGroup = TRUE;
	m_bUndoBeginGroup = m_nUndoPosition == 0 || ! bMergeWithPrevious;
	//TRACE("\nBeginUndoGroup = %d, m_nUndoPosition=%d ",m_bUndoBeginGroup,m_nUndoPosition);
}
...
void CCrystalTextBuffer::FlushUndoGroup(CCrystalTextView *pSource)
{
	...
	if (pSource != NULL)
	{
		...
		TRACE("\nFlushing Record : m_aUndoBuf.GetSize()=%d, m_nUndoPosition =%d", m_nUndoPosition , m_aUndoBuf.GetSize());
		
		if (m_nUndoPosition < m_aUndoBuf.GetSize()) {

			m_bUndoGroup = m_savedUndoGroup;
			m_bUndoBeginGroup = m_savedUndoBeginGroup;
			return;
		}
