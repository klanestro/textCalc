...
COLORREF CCrystalTextView::GetColor(int nColorIndex)
{
	...
	case COLORINDEX_SELBKGND:
		//return RGB(0, 0, 0);
		//return RGB(200, 250, 250);
		return RGB(180, 225, 225);
...
void CCrystalTextView::OnBeginPrinting(CDC *pdc, CPrintInfo *pInfo)
{
	...
	TRACE("\nAfWidth=%d", lf.lfWidth);
	TRACE("\nAfHeight=%d", lf.lfHeight);
...
void CCrystalTextView::OnUpdateIndicatorPosition(CCmdUI* pCmdUI)
{
	...
	stat.Format( _T("Ln %d, Col %d"), m_ptCursorPos.y + 1,m_nIdealCharPos + 1 );  // <-- modified
...
BOOL CCrystalTextView::FindSelection(BOOL bDirection) //[MP]
...
// Section: select current word
		...
		if (pt.y < ptAnchor.y ||
			pt.y == ptAnchor.y && pt.x < ptAnchor.x)
		{
...
// End section
	...
	if (pApp->GetProfileInt(REG_FIND_SUBKEY, REG_MATCH_CASE, FALSE))
		dwSearchFlags |= FIND_MATCH_CASE;
	if (pApp->GetProfileInt(REG_FIND_SUBKEY, REG_WHOLE_WORD, FALSE))
		dwSearchFlags |= FIND_WHOLE_WORD;
	if (bDirection==1)
		dwSearchFlags |= FIND_DIRECTION_UP;
	CPoint ptTextPos;
	if (!FindTextInBlock(sSearchText, GetCursorPos(), CPoint(0, 0),
			CPoint(GetLineLength(nLineCount - 1), nLineCount - 1),
			dwSearchFlags, TRUE, &ptTextPos))
		return FALSE;
	m_bLastSearch = TRUE;
	if (m_pszLastFindWhat != NULL)
		free(m_pszLastFindWhat);
...
BOOL CCrystalTextView::GoToLine(int nLine) //[MP]
{
	if (nLine>=m_pTextBuffer->GetLineCount())
		nLine = m_pTextBuffer->GetLineCount()-1;
	if (nLine >= 0)
	{
		CPoint pt(0, nLine);
		ASSERT_VALIDTEXTPOS(pt);
...
CString CCrystalTextView::GetSelectedText() 
{
