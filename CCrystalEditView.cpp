...
void CCrystalEditView::PasteSelection(LPCTSTR pszNewText,int partial)
{
	...
	if (partial==0) { //need to select all lines

		CPoint sStart,sEnd;
		GetSelection(sStart, sEnd);
		
		int nLength = GetLineLength(sEnd.y);
		sStart.x=0;
		sEnd.x=nLength;
...
void CCrystalEditView::BeginUnitPaste()
{
	if (! QueryEditable())
		return;
	if (m_pTextBuffer == NULL)
...
void CCrystalEditView::EndUnitPaste()
{
	if (! QueryEditable())
		return;
	if (m_pTextBuffer == NULL)
...
void CCrystalEditView::UnitPasteSelection(LPCTSTR pszNewText)
{
	if (! QueryEditable())
		return;
	if (m_pTextBuffer == NULL)
